function [ns1, eyeTrackdata, log, exitflag] = NS_present_nswitch_v03(group, ptb, log, colortype, indexletter, ISI, eyeTrackdata)
...
% if strcmp(ptb.hostname, cenir_hostname)  % send triggers to eyelink at cenir
%     message = 0; % integer from 0 to 255
...
for i_trial = 1 : length(colortype)
  ...
  %  end
    ...
    %if strcmp(ptb.hostname, cenir_hostname)  % send triggers to eyelink at cenir
    %    message = 42; % integer from 0 to 255
    %    WriteParPort(message);
    %end
    ...
    %if strcmp(ptb.hostname, cenir_hostname)  % send triggers to eyelink at cenir
    %    message = 43; % integer from 0 to 255
    %    WriteParPort(message);
    %end
...
%     if strcmp(ptb.hostname, cenir_hostname)  % send triggers to eyelink at cenir
%         message = 45; % integer from 0 to 255
%         WriteParPort(message);
%     end
